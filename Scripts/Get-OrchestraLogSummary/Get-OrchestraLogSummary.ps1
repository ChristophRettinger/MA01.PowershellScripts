...
.EXAMPLE
    .\Get-OrchestraLogSummary.ps1 -LogPath ".\server.log"

.EXAMPLE
    .\Get-OrchestraLogSummary.ps1 -LogPath ".\logs\*.log" -OutputDirectory ".\out" -SettingsFile ".\normalize.txt"
#>
[CmdletBinding()]
param (
...
if (-not [string]::IsNullOrWhiteSpace($OutputDirectory)) {
    ...
    $csvPath = Join-Path -Path $OutputDirectory -ChildPath "Get-OrchestraLogSummary_$($stamp).csv"
    $txtPath = Join-Path -Path $OutputDirectory -ChildPath "Get-OrchestraLogSummary_$($stamp).txt"
