...
- **Resend-FromElastic.ps1** - Queries SUBFL records from Elasticsearch, keeps only the oldest hit per BusinessCaseId/MSGID, supports grouped reporting, controlled resend/test replay, curl command export for configured HTTP targets, filters by WorkflowPattern (custom value or ErrorOnly shortcut), converts operator-supplied StartDate/EndDate from local time to UTC for `@timestamp` filtering, and offers a `ShowQuery` action to print the Elasticsearch request body without executing it.
