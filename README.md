...
- **Get-DeploymentInfo.ps1** - Calls Orchestra deployment APIs per server, supports version and landscape comparisons with full server-alias headers and missing-value placeholders, supports optional scenario auto-rename alignment and JDBC URL visibility in landscape mode, and caches per-server credentials.
