...
- **Create-ProcessModelOverview.ps1** - Parses ProcessModell_* XML files and creates Markdown overviews for model metadata, variables, business keys, merged element detail rows (input assignments, output assignments, parameters), and gateway outgoing edge conditions, writing reports to the script-local `Output` folder by default.
