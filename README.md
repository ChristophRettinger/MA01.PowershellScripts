...
- **Send-HL7Message.ps1** - Sends HL7 payloads over MLLP from either a UTF-8 input file or a dynamically built query request (`-Message`, `-AID`, optional `-Sender`), shows the outbound request plus colorized responses, and supports `Tcp`, `Tls`, or `TlsWithCertificate`.
